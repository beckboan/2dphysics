...
void Engine::addTestParams() const {
  ...
  std::vector<vec2d> box_verts = {vec2d(0, 0), vec2d(100, 0), vec2d(0, 100),
                                  vec2d(100, 100)};

  std::vector<vec2d> verticies_2 = {vec2d(0, 0),  vec2d(10, 0),  vec2d(10, 10),
                                    vec2d(0, 10), vec2d(15, 20), vec2d(-10, 5)};
  std::vector<vec2d> verticies_3 = {vec2d(0, 0), vec2d(10, 0), vec2d(10, 10),
...
void Engine::addLevelParams(const std::string &filename) {
  ...
  try {
    ...
    if (config["bodies"].IsNull()) {
      std::cerr << "Error: No bodies or boundaries found in the file"
                << std::endl;
    ...
    for (const auto &body : config["bodies"]) {
      try {
        ...
        if (shape_type == "circle") {
          ...
          auto temp_position = vec2d(body["position"][0].as<float>(),
                                     body["position"][1].as<float>());
          ...
          world->addCircle(temp_radius, temp_position, temp_density,
                           temp_static);
          ...
          for (const auto &vertex : body["points"]) {
            temp_verts.emplace_back(vertex[0].as<float>(),
                                    vertex[1].as<float>());
          }
    ...
    for (const auto &boundary : config["boundaries"]) {
      auto temp_start = vec2d(boundary["start"][0].as<float>(),
                              boundary["start"][1].as<float>());
      auto temp_end =
          vec2d(boundary["end"][0].as<float>(), boundary["end"][1].as<float>());
      try {
    ...
    std::cerr << "Error: Exception while loading the YAML file - " << e.what()
              << std::endl;
  }
}
